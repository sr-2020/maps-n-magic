...
import { MockedReadDataManager } from './MockedReadDataManager';
...
export class MockedCrudDataManager<
  Entity extends Identifiable
> extends MockedReadDataManager<Entity> {
  ...
  constructor(
    ...
    super(gameModel, entities, entityName, readStrategy, logger);
  ...
  onPostEntityRequested({ props }: {props: Entity}) {
    const entity = { ...props };
    let id = this.entities.length === 0 ? 1 : this.entities.reduce((acc, entity) => {
      if (acc < entity.id ) {
        acc = entity.id;
      }
      return acc;
